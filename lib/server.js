...
function init(app, options = {}) {
  ...
  console.log(`init with latency: ${latency}, webroot: ${webroot}`);
}
...
function all(webroot) {
  return async function all(ctx) {
    ...
    if (error != null || missing != null) {
      ...
      console.log(`not ok: ${ctx.path} responding with ${ctx.status}`);
      return;
    }
    ...
    if ('service-worker' in ctx.request.headers) {
      if (body) {
        console.warn(`not ok: ${ctx.path} Service Worker not found`);
    ...
    console.log(body ? `ok: ${ctx.path} responding with dummy file` : `ok: ${ctx.path} responding with file`);
  };
}
